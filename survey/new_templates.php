...
<option value="<?php echo htmlentities($row1->category);?> "><?php echo htmlentities($row1->category);?> </option>
 <?php } ?> 
					  ...
					  <div class="col-md-6">
                        ...
                        <select  name="user_id" class="form-control form-control-sm" required='true'>
                          <option value=""><?php echo isset($user_id) ? $user_id : '' ?></option>
                         <?php 
...
{          
    ?>  
<option value="<?php echo htmlentities($row1->id);?> "><?php echo htmlentities($row1->fullname);?></option>
 <?php } ?> 
                        </select>
                      </div>
				</div>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						location.replace('index.php?page=templates_list')
					},1500)
				}
			}
		})
